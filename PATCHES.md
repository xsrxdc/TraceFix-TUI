...
| 17 | TraceFix TUI README (replace upstream opencode front page) | README.md | repo front page was stock opencode (branding, npm/curl install, dead localized-README links); now describes the TraceFix TUI fork, its relation to the platform repo, build, and upstream-sync discipline | no |
| 18 | file context OFF by default | component/prompt/index.tsx, app.tsx (kv `file_context_enabled` default) | the design TUI takes a natural-language requirement, not the IDE's selected/active file — injecting that file into the prompt is noise for the designer. Flips the `file_context_enabled` default true→false in the prompt gate and the toggle command; still switchable via the "Enable file context" command. | partly (sensible default for a design-only TUI) |
